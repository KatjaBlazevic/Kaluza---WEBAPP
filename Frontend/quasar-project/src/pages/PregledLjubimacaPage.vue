...
async function fetchLjubimci() {
  ...
  try {
    ...
    if (!res.ok) throw new Error('Neuspješno dohvaćanje ljubimaca.');
...
// 🔹 Funkcija koja preusmjerava na stranicu za uređivanje ljubimca
...
async function obrisiLjubimca(id) {
  ...
  try {
    ...
    if (!res.ok) throw new Error('Neuspješno brisanje ljubimca.');
